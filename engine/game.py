...
class GameRound:
    ...
    def next_turn(self):
        ...
        if self.visual_mode:
            ...
            if self.gui_.stop_game.get() == True:
        ...
        curr_player_move_code = 0
        # player must conclude actions (code 1) to end pre roll phase
        while curr_player_move_code != 1:
            curr_player_move_code = current_player.agent.make_pre_roll_moves()
        ...
        if current_player.money < 0:
            if current_player.agent.handle_negative_balance() == False:
                ...
                # if not don't update current_player_id because after bakruptcy is already points to the next player
